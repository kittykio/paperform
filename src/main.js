...
function controls() {
  ...
  $("#controls").innerHTML =
    tab === "design"
      ? `<div class="control-heading"><span>01</span><h2>Your message</h2></div><label>Small introduction<input data-key="eyebrow" maxlength="160" value="${esc(s.eyebrow)}"></label><label>The big idea <span>MAKE IT PERSONAL</span><textarea data-key="title" rows="3" maxlength="90">${esc(s.title)}</textarea></label><label>A little more to say<textarea data-key="note" rows="2" maxlength="160">${esc(s.note)}</textarea></label><div class="control-heading"><span>02</span><h2>Type with personality</h2></div><div class="segmented">${[
          ["serif", "Editorial"],
          ["sans", "Bold"],
          ["mono", "Typewriter"],
        ]
          .map(
            ([v, l]) =>
              `<button data-font="${v}" class="${s.font === v ? "selected" : ""}">${l}</button>`,
          )
          .join(
            "",
      ...
      : `<div class="control-heading"><span>01</span><h2>Pick your paper</h2></div><div class="swatches">${palettes.map((p, i) => `<button data-palette="${i}" aria-label="${p.name}" title="${p.name}" class="${s.palette === i ? "selected" : ""}" style="background:${p.paper};color:${p.ink}">Aa</button>`).join("")}</div><p class="fine">${palettes[s.palette].name}</p><label>Paper texture<select data-key="pattern">${["plain", "dots", "grid"].map((v) => `<option ${s.pattern === v ? "selected" : ""}>${v}</option>`).join("")}</select></label><div class="control-heading"><span>02</span><h2>A different dimension</h2></div><div class="fold-options">${[
...
function render() {
  ...
  card.dataset.pattern = state.pattern;
...
document.addEventListener("click", (e) => {
  ...
  if (b.dataset.template !== undefined) {
    stop();
    state = { ...defaults, ...templates[+b.dataset.template] };
    controls();
    render();
    save();
...
function artwork() {
  ...
  ctx.strokeStyle = p.ink + "22";
  ctx.fillStyle = p.ink + "33";
  ...
  text(
    ...
    350,
    size,
    `bold SIZEpx ${state.font === "serif" ? "Georgia" : state.font === "mono" ? "monospace" : "Arial"}`,
    1350,
  );
